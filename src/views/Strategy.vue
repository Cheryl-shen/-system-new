<template>
  <div class="strategy-page">
    <div class="page-header">
      ...
      <h2>客户战略分析</h2>
      <p>重点客户经营情况 · AI 规划 · 出海战略 · 最新财报 · AI产品售卖指引（定期更新）</p>
    </div>
    ...
    <div class="page-tabs">
      <div
        class="page-tab"
        :class="{ active: pageTab === 'customers' }"
        @click="pageTab = 'customers'"
      >
        <span class="page-tab-icon">👥</span>客户分析
      </div>
      <div
        class="page-tab"
        :class="{ active: pageTab === 'product-guide' }"
        @click="pageTab = 'product-guide'"
      >
        <span class="page-tab-icon">📦</span>AI产品售卖指引
      </div>
    </div>

    <!-- ========== 客户分析 ========== -->
    <div v-if="pageTab === 'customers'">
    ...
    <!-- ========== AI产品售卖指引 ========== -->
    <div v-if="pageTab === 'product-guide'" class="product-guide-section">

      <!-- 目录导航 -->
      <div class="info-card" style="margin-bottom: 16px">
        <div class="info-card-header" :class="{ collapsed: !guideTocExpanded }" @click="guideTocExpanded = !guideTocExpanded">
          <h3>📜 目录导航</h3>
          <span class="toggle-icon">▼</span>
        </div>
        <div class="info-card-body" :class="{ hidden: !guideTocExpanded }">
          <div class="toc-list">
            <span class="info-tag tag-blue" @click="scrollToGuideSection('guide-section-1')">1. 大背景</span>
            <span class="info-tag tag-blue" @click="scrollToGuideSection('guide-section-2')">2. 行业分析</span>
            <span class="info-tag tag-blue" @click="scrollToGuideSection('guide-section-3')">3. 产品全景</span>
            <span class="info-tag tag-blue" @click="scrollToGuideSection('guide-section-4')">4. 售卖案例</span>
          </div>
        </div>
      </div>

      <!-- 1. 大背景 -->
      <div class="info-card" id="guide-section-1">
        <div class="info-card-header" :class="{ collapsed: !guideSection.background }" @click="guideSection.background = !guideSection.background">
          <h3>
            📜 1. 大背景：现在是最佳窗口
        ...
        <div class="info-card-body" :class="{ hidden: !guideSection.background }">
      ...
      <!-- 2. 行业分析 -->
      <div class="info-card" id="guide-section-2">
        <div class="info-card-header" :class="{ collapsed: !guideSection.industry }" @click="guideSection.industry = !guideSection.industry">
          <h3>
            📊 2. 行业宏观分析 + AI切入方向
          </h3>
          <span class="toggle-icon">▼</span>
        </div>
        <div class="info-card-body" :class="{ hidden: !guideSection.industry }">
          <p class="section-desc">不同行业有不同AI应用场景和切入点，需根据客户行业特征选择合适的AI产品组合</p>

          <div class="sheet-container">
            <div class="sheet-tabs">
              <div
                v-for="industry in guideIndustries"
                :key="industry.key"
                class="sheet-tab"
                :class="{ active: guideActiveIndustry === industry.key }"
                @click="guideActiveIndustry = industry.key"
              >
                {{ industry.name }}
              </div>
            </div>
            ...
            <div class="sheet-panel active">
              <div class="industry-desc">{{ guideCurrentIndustry.desc }}</div>
              <table class="data-table full-width">
                ...
                <tr v-for="(row, idx) in guideCurrentIndustry.rows" :key="idx">
                  <td>{{ row.situation }}</td>
                  <td>{{ row.direction }}</td>
                  <td>{{ row.products }}</td>
                </tr>
      ...
      <!-- 3. 产品全景 -->
      <div class="info-card" id="guide-section-3">
        <div class="info-card-header" :class="{ collapsed: !guideSection.products }" @click="guideSection.products = !guideSection.products">
          <h3>
            📜 3. 腾讯云AI产品全景
          </h3>
          <span class="toggle-icon">▼</span>
        </div>
        <div class="info-card-body" :class="{ hidden: !guideSection.products }">
          <!-- 产品总览 -->
      ...
      <!-- 4. 售卖案例 -->
      <div class="info-card" id="guide-section-4">
        <div class="info-card-header" :class="{ collapsed: !guideSection.cases }" @click="guideSection.cases = !guideSection.cases">
          <h3>
            📜 4. 售卖案例
          </h3>
          <span class="toggle-icon">▼</span>
        </div>
        <div class="info-card-body" :class="{ hidden: !guideSection.cases }">
          <div class="sheet-container">
            <div class="sheet-tabs">
              <div
                v-for="caseItem in guideCases"
                :key="caseItem.key"
                class="sheet-tab"
                :class="{ active: guideActiveCase === caseItem.key }"
                @click="guideActiveCase = caseItem.key"
              >
                {{ caseItem.title }}
              </div>
            </div>
            <div class="sheet-panel active">
              <div class="case-background">{{ guideCurrentCase.background }}</div>
              <div class="two-col-grid small-text" style="margin: 12px 0">
                <div>
                  <strong>策略维度：</strong><br>
                  <span v-html="guideCurrentCase.strategy"></span>
                </div>
                <div>
                  <strong>关键经验：</strong><br>
                  <span v-html="guideCurrentCase.experience"></span>
                </div>
              </div>
              <div class="result-box">
                <strong>结果：</strong><br>
                <span v-html="guideCurrentCase.result"></span>
              </div>
            </div>
          </div>
        </div>
...
// 页面级 Tab 状态（支持从路由 query 跳转）
const pageTab = ref<'customers' | 'product-guide'>(
  route.query.tab === 'product-guide' ? 'product-guide' : 'customers'
)
...
// ========== AI产品售卖指引 数据 ==========
const guideTocExpanded = ref(true)
const guideSection = reactive({
  background: true,
  industry: true,
  products: true,
  cases: true
...
const guideActiveIndustry = ref('dianshang')
const guideIndustries = [
  {
    key: 'dianshang',
    name: '电商行业',
...
const guideCurrentIndustry = computed(() => {
  return guideIndustries.find(i => i.key === guideActiveIndustry.value) || guideIndustries[0]
})

const guideActiveCase = ref('case1')
const guideCases = [
  {
    key: 'case1',
    title: '案例一：Codebuddy',
...
const guideCurrentCase = computed(() => {
  return guideCases.find(c => c.key === guideActiveCase.value) || guideCases[0]
})
...
/* ========== AI产品售卖指引 样式 ========== */
.product-guide-section .info-card {
  background: var(--bg-white);
  border: 1px solid var(--border);
  border-radius: var(--radius);
  margin-bottom: 16px;
...
.product-guide-section .info-card-header {
  padding: 14px 20px;
  display: flex;
  align-items: center;
...
.product-guide-section .info-card-header:hover {
  background: var(--bg);
}

.product-guide-section .info-card-header h3 {
  font-size: 14px;
  font-weight: 600;
  display: flex;
  align-items: center;
...
.product-guide-section .info-card-body {
  padding: 0 20px 16px;
  font-size: 13px;
  color: var(--text-secondary);
  line-height: 1.8;
...
.product-guide-section .info-card-body.hidden {
  display: none;
}
...
.product-category-card.blue { background: #f0f9ff; border: 1px solid #bae6fd; }
.product-category-card.purple { background: #f5f3ff; border: 1px solid #c7d2fe; }
.product-category-card.yellow { background: #fff7ed; border: 1px solid #fcd34d; }
.product-category-card.red { background: #fef2f2; border: 1px solid #fecaca; }
...
.product-detail-box.purple-light { background: #f8f5ff; border: 1px solid #d9d1ff; }
.product-detail-box.blue-light { background: #eef2ff; border: 1px solid #bae6fd; }
.product-detail-box.yellow-light { background: #fff7ed; border: 1px solid #fcd34d; }
.product-detail-box.green-light { background: #f0fdf4; border: 1px solid #bbf7d0; }
